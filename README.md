...
私教会根据经质量门禁筛选的历年考频和你的真实作答记录，持续知道你学过什么、错在哪里、何时需要复习，以及下一小时学习什么最能提高三科同时过线的概率。

- **只求过线**：三科分别以 45 分为合格线，日常训练瞄准 50–55 分安全区，不追求高分。
- **先抓高频**：不按教材顺序平均用力，优先历年常考、容易提分和三科复用的内容。
- **主动强化**：诊断 → 微课 → 做题 → 错因 → 变式 → 间隔复习；微课以题目解析为事实素材，错因只记录考生明确提供的信息，不靠“看过了”冒充掌握。
- **进度有证据**：一次客观题答对不算掌握；综合、案例、论文分别按各自门槛记录识别、应用和产出能力。
- **默认私密**：个人档案、错题和论文项目只保存在 Git 忽略的 `.study/`，不会提交到公共仓库。

30 秒开始、支持的说法和隐私说明见 [`tutor/README.md`](./tutor/README.md)。
考频当前采用“人工复核稳定层 + 近年回忆版趋势层”的双层审计口径；在映射覆盖率达到门槛前，新的统计保持报告模式，不会直接改写排课权重，详见 [`tutor/frequency-snapshot.json`](./tutor/frequency-snapshot.json)。
